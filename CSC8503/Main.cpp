...
void TestPathfinding() {
	NavigationGrid grid("TestGrid1.txt", {0,0,0});
...
void TestNetworking()
{
	...
	for (int i = 0; i < 100; i++)
	{
		StringPacket sp("Server says hello! " + std::to_string(i));
		server->SendGlobalPacket(sp);
		StringPacket cp(StringPacket("Client says hello! " + std::to_string(i)));
