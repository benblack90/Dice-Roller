...
PhysicsObject::PhysicsObject(Transform* parentTransform, const CollisionVolume* parentVolume)	{
	...
	friction	= 0.8f;
	useGravity = true;
	frameAngularDampingCoeff = frameLinearDampingCoeff = 0.4f;
}
...
void PhysicsObject::InitD8Inertia()
{
	//1/10 ms^2
	float height = transform->GetScale().GetMaxElement();
	float sideLength = (height / 2.0f) / cosf(45.0f);
	inverseInertia.x = inverseInertia.y = inverseInertia.z = (10.0f * inverseMass) / (sideLength * sideLength);
