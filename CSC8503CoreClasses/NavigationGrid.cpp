...
NavigationGrid::NavigationGrid(const std::string&filename, const Vector3 topLeft) : NavigationGrid() {
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			infile >> type;
			n.type = type;
			n.position = Vector3((float)(x * nodeSize + topLeft.x), 0, (float)(y * nodeSize + topLeft.z));
			if (type == 'L') lockButton = n.position;
			if (type == 'E') exitDefButton = n.position;
			if (type == 'M') mazeDefButton = n.position;
		}
	}
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			for (int i = 0; i < 4; ++i) {
				if (n.connected[i]) {
					if (n.connected[i]->type == '.' || n.connected[i]->type == 'L' || n.connected[i]->type == 'E' || n.connected[i]->type == 'M') {
						n.costs[i]		= 1;
					}
...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	int fromX = (((int)from.x - topLeft.x)/ nodeSize);
	int fromZ = (((int)from.z - topLeft.z)/ nodeSize);

	int toX = (((int)to.x - topLeft.x)/ nodeSize);
	int toZ = (((int)to.z - topLeft.z)/ nodeSize);
